...
const closeAddRocketModalButton = document.getElementById(
	"add-rocket-modal-close-button"
);
const closeUpdateRocketModalButton = document.getElementById(
	"update-rocket-modal-close-button"
);
...
const updateRocketModalForm = document.getElementById(
	"update-rocket-modal-form"
);
...
document.addEventListener("DOMContentLoaded", () => {
	if (addRocketButton)
		addRocketButton.addEventListener(
			"click",
			() => (addRocketModal.style.visibility = "visible")
		);
	if (closeAddRocketModalButton)
		closeAddRocketModalButton.addEventListener(
			"click",
			() => (addRocketModal.style.visibility = "hidden")
		);
	if (addRocketModalForm)
		addRocketModalForm.addEventListener("submit", (event) =>
			addRocket(event)
		);

	if (closeUpdateRocketModalButton)
		closeUpdateRocketModalButton.addEventListener(
			"click",
			() => (updateRocketModal.style.visibility = "hidden")
		);
	if (updateRocketModalForm)
		updateRocketModalForm.addEventListener("submit", (event) =>
			updateRocket(event)
		);

	if (rocketNotification)
		rocketNotification.addEventListener("click", () => {
			rocketNotification.style.top = "-100%";
		});
		...
		.then(async (response) => {
			if (response.ok) {
				...
				errorNotification(
					rocketNotification,
					"rocket-notification",
					"Sorry, rocket could not be added!"
				);
		})
		.catch(() =>
			errorNotification(
