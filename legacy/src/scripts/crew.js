...
const closeAddCrewModalButton = document.getElementById(
	"add-crew-modal-close-button"
);
const closeUpdateCrewModalButton = document.getElementById(
	"update-crew-modal-close-button"
);
...
document.addEventListener("DOMContentLoaded", () => {
	if (addCrewButton)
		addCrewButton.addEventListener(
			"click",
			() => (addCrewModal.style.visibility = "visible")
		);
	if (closeAddCrewModalButton)
		closeAddCrewModalButton.addEventListener(
			"click",
			() => (addCrewModal.style.visibility = "hidden")
		);
	if (addCrewModalForm)
		addCrewModalForm.addEventListener("submit", (event) => addCrew(event));

	if (closeUpdateCrewModalButton)
		closeUpdateCrewModalButton.addEventListener(
			"click",
			() => (updateCrewModal.style.visibility = "hidden")
		);
	if (updateCrewModalForm)
		updateCrewModalForm.addEventListener("submit", (event) =>
			updateCrew(event)
		);

	if (crewNotification)
		crewNotification.addEventListener("click", () => {
			crewNotification.style.top = "-100%";
		});
...
function renderCrew(parentDiv, crew, crewClasses, path, shouldRenderButtons) {
	...
	crew.crewmans.forEach((crewman) =>
		renderCrewman(contentDiv, crewman, ["sub-list-item"], false)
	);
...
async function addCrew(event) {
	...
	const createCrew = {
		...
		crewmans: await JSON.parse(
			`[${document.getElementById("add-crew-form-crewmans").value}]`
		),
	};
...
async function updateCrew(event) {
	...
	const createCrew = {
		...
		crewmans: await JSON.parse(
			`[${document.getElementById("update-crew-form-crewmans").value}]`
		),
	};
		...
		.then(async (response) => {
			if (response.ok) {
				...
				errorNotification(
					crewNotification,
					"crew-notification",
					"Sorry, crew could not be updated!"
				);
		})
		.catch(() =>
			errorNotification(
		...
		.then(async (response) => {
			if (response.ok) {
				...
				errorNotification(
					crewNotification,
					"crew-notification",
					"Sorry, crew could not be deleted!"
				);
		})
		.catch(() =>
			errorNotification(
...
function editUpdateCrewModal(crew) {
	...
	document.getElementById("update-crew-form-crewmans").value = crew.crewmans
		.map((crewman) => crewman.id)
		.join(", ");
