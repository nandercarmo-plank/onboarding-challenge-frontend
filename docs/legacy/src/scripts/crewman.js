...
const closeAddCrewmanModalButton = document.getElementById(
	"add-crewman-modal-close-button"
);
const closeUpdateCrewmanModalButton = document.getElementById(
	"update-crewman-modal-close-button"
);
...
const updateCrewmanModalForm = document.getElementById(
	"update-crewman-modal-form"
);
...
document.addEventListener("DOMContentLoaded", () => {
	if (addCrewmanButton)
		addCrewmanButton.addEventListener(
			"click",
			() => (addCrewmanModal.style.visibility = "visible")
		);
	if (closeAddCrewmanModalButton)
		closeAddCrewmanModalButton.addEventListener(
			"click",
			() => (addCrewmanModal.style.visibility = "hidden")
		);
	if (addCrewmanModalForm)
		addCrewmanModalForm.addEventListener("submit", (event) =>
			addCrewman(event)
		);

	if (closeUpdateCrewmanModalButton)
		closeUpdateCrewmanModalButton.addEventListener(
			"click",
			() => (updateCrewmanModal.style.visibility = "hidden")
		);
	if (updateCrewmanModalForm)
		updateCrewmanModalForm.addEventListener("submit", (event) =>
			updateCrewman(event)
		);

	if (crewmanNotification)
		crewmanNotification.addEventListener("click", () => {
			crewmanNotification.style.top = "-100%";
		});

	renderCrewmanList(crewmanList);
});

function renderCrewmanList(
	crewmanListDiv,
	path = "../../assets/",
...
function renderCrewman(
	...
	crewmanClasses.forEach((crewmanClass) =>
		crewmanDiv.classList.add(crewmanClass)
	);
		...
		.then(async (response) => {
			if (response.ok) {
				...
				errorNotification(
					crewmanNotification,
					"crewman-notification",
					"Sorry, crewman could not be added!"
				);
		})
		.catch(() =>
			errorNotification(
