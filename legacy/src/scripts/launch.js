...
const closeAddLaunchModalButton = document.getElementById(
	"add-launch-modal-close-button"
);
const closeUpdateLaunchModalButton = document.getElementById(
	"update-launch-modal-close-button"
);
...
const updateLaunchModalForm = document.getElementById(
	"update-launch-modal-form"
);
...
document.addEventListener("DOMContentLoaded", () => {
	if (addLaunchButton)
		addLaunchButton.addEventListener(
			"click",
			() => (addLaunchModal.style.visibility = "visible")
		);
	if (closeAddLaunchModalButton)
		closeAddLaunchModalButton.addEventListener(
			"click",
			() => (addLaunchModal.style.visibility = "hidden")
		);
	if (addLaunchModalForm)
		addLaunchModalForm.addEventListener("submit", (event) =>
			addLaunch(event)
		);

	if (closeUpdateLaunchModalButton)
		closeUpdateLaunchModalButton.addEventListener(
			"click",
			() => (updateLaunchModal.style.visibility = "hidden")
		);
	if (updateLaunchModalForm)
		updateLaunchModalForm.addEventListener("submit", (event) =>
			updateLaunch(event)
		);

	if (launchNotification)
		launchNotification.addEventListener("click", () => {
			launchNotification.style.top = "-100%";
		});
...
function addLaunch(event) {
	...
	const createLaunch = {
		...
		success:
			document.getElementById("add-launch-form-success").value === "true",
		rocketId: parseInt(
			document.getElementById("add-launch-form-rocket").value
		),
		...
		.then(async (response) => {
			if (response.ok) {
				...
				errorNotification(
					launchNotification,
					"launch-notification",
					"Sorry, launch could not be added!"
				);
		})
		.catch(() =>
			errorNotification(
...
function updateLaunch(event) {
	...
	const createLaunch = {
		launchCode: document.getElementById("update-launch-form-launch-code")
			.value,
		date: document.getElementById("update-launch-form-date").value,
		success:
			document.getElementById("update-launch-form-success").value === "true",
		rocketId: parseInt(
			document.getElementById("update-launch-form-rocket").value
		),
		crewId: parseInt(
			document.getElementById("update-launch-form-crew").value
		),
	};
...
function editUpdateLaunchModal(launch) {
	...
	document.getElementById("update-launch-form-launch-code").value =
		launch.launchCode;
	document.getElementById("update-launch-form-date").value = launch.date;
	document.getElementById("update-launch-form-success").value = launch.success
		? "true"
		: "false";
	document.getElementById("update-launch-form-rocket").value =
		launch.rocket.id;
